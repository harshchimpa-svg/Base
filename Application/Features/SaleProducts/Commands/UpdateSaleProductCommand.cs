...
internal class UpdateSaleProductCommandHandler : IRequestHandler<UpdateSaleProductCommand, Result<SaleProduct>>
{
    ...
    {
        var saleProduct = await _unitOfWork.Repository<SaleProduct>().Entities.FirstOrDefaultAsync(x => x.Id == request.Id);

        if (saleProduct == null)
        {
            return Result<SaleProduct>.BadRequest("SaleProduct id not found");
        }

        _mapper.Map(request.CreateCommand, saleProduct);

        await _unitOfWork.Repository<SaleProduct>().UpdateAsync(saleProduct);
