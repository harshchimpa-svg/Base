...
internal class UpdateExerciseCommandHandler : IRequestHandler<UpdateExerciseCommand, Result<Exercise>>
{
    ...
    public async Task<Result<Exercise>> Handle(UpdateExerciseCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            var dietType = await _unitOfWork.Repository<Exercise>().GetByID(request.CreateCommand.DietTypeId.Value);
        ...
        var exercise = await _unitOfWork.Repository<Exercise>().Entities.FirstOrDefaultAsync(x => x.Id == request.Id);
        {
            ...
            _mapper.Map(request.CreateCommand, exercise);

            await _unitOfWork.Repository<Exercise>().UpdateAsync(exercise);
