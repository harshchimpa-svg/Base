...
internal class UpdateSalePaymentCommandHandler : IRequestHandler<UpdateSalePaymentCommand, Result<SalePayment>>
{
    ...
    {
        var salePayment = await _unitOfWork.Repository<SalePayment>().Entities.FirstOrDefaultAsync(x => x.Id == request.Id);

        if (salePayment == null)
        {
            return Result<SalePayment>.BadRequest("Sale Payment id not found");
        }

        _mapper.Map(request.CreateCommand, salePayment);

        await _unitOfWork.Repository<SalePayment>().UpdateAsync(salePayment);
        await _unitOfWork.Save(cancellationToken);

        return Result<SalePayment>.Success("Update SalePayments...");
    }
}